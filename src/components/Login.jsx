  ...
  return (
    <div className="flex items-center justify-center w-full">
      ...
      >
        ...
        <form onSubmit={handleSubmit(login)} className="mt-8"> {/*On submit it itself calls the callback function login that we defined above*/}
          <div className="space-y-5">
            <Input
              label="Email: "
              placeholder="Enter your email"
              type="email"
              {...register("email", { //this register function is used to register the input field with react-hook-form. So that the value enetered is accessible in the form data.
                required: true,
                validate: {
                  matchPatern: (value) =>
                    /^\w+([.-]?\w+)*@\w+([.-]?\w+)*(\.\w{2,3})+$/.test(value) || //this tests if the entered email matches the email format
