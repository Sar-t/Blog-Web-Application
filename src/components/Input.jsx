...
const Input = React.forwardRef(function Input({ //forwarding the ref to the input element 
  //also to do integration with react-hook-form, we can use the ref to register the input element
   ...
   return (<div className='w-full'>
    {label && <label className='inline-block text-white mb-1 pl-1' id={id}>{label}</label>
    }
    <input
    type = {type}
    className={`px-3 py-2 text-white rounded-lg bg-blend-color outline-none focus: duration-200 border border-gray-200 w-full ${className}`}
